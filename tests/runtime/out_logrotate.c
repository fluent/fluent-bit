...
void flb_test_logrotate_gzip_compression_exact_chunk(void);
...
void flb_test_logrotate_format_msgpack(void);
void flb_test_logrotate_format_template(void);
void flb_test_logrotate_path(void);
void flb_test_logrotate_mkdir(void);
void flb_test_logrotate_delimiter(void);
void flb_test_logrotate_label_delimiter(void);
void flb_test_logrotate_csv_column_names(void);
void flb_test_logrotate_multithreaded(void);

/* Test list */
TEST_LIST = {{"basic_rotation", flb_test_logrotate_basic_rotation},
             ...
             {"format_msgpack", flb_test_logrotate_format_msgpack},
             {"format_template", flb_test_logrotate_format_template},
             {"path", flb_test_logrotate_path},
             {"mkdir", flb_test_logrotate_mkdir},
             {"delimiter", flb_test_logrotate_delimiter},
             {"label_delimiter", flb_test_logrotate_label_delimiter},
             {"csv_column_names", flb_test_logrotate_csv_column_names},
             {"multithreaded", flb_test_logrotate_multithreaded},
             {NULL, NULL}};
...
void flb_test_logrotate_format_ltsv(void) {
  ...
  TEST_CHECK(out_ffd >= 0);
  TEST_ASSERT(flb_output_set(ctx, out_ffd, "match", "test", "file", logfile,
                             "format", "ltsv", "max_size", "100M", "gzip",
                             "false", NULL) == 0);
...
void flb_test_logrotate_format_plain(void) {
  ...
  TEST_CHECK(out_ffd >= 0);
  TEST_ASSERT(flb_output_set(ctx, out_ffd, "match", "test", "file", logfile,
                             "format", "plain", "max_size", "100M", "gzip",
                             "false", NULL) == 0);
...
void flb_test_logrotate_format_msgpack(void) {
  ...
  TEST_CHECK(out_ffd >= 0);
  TEST_ASSERT(flb_output_set(ctx, out_ffd, "match", "test", "file", logfile,
                             "format", "msgpack", "max_size", "100M", "gzip",
                             "false", NULL) == 0);
  ...
  if (stat(logfile, &st) == 0) {
    ...
    /* Msgpack files should not be readable as text (no newlines in first bytes)
     */
    fp = fopen(logfile, "rb");
    if (fp) {
...
void flb_test_logrotate_format_template(void) {
  ...
  recursive_delete_directory(TEST_LOGPATH);
  mkdir(TEST_LOGPATH, 0755);
  snprintf(logfile, sizeof(logfile), "%s/%s", TEST_LOGPATH,
           "test_template.log");
  ...
  TEST_ASSERT(flb_output_set(ctx, out_ffd, "match", "test", "file", logfile,
                             ...
                             "{time} {message}", "max_size", "100M", "gzip",
                             "false", NULL) == 0);
  ...
  for (i = 0; i < 3; i++) {
    bytes =
        flb_lib_push(ctx, in_ffd, (char *)json_template, strlen(json_template));
...
void flb_test_logrotate_path(void) {
  ...
  snprintf(test_path, sizeof(test_path), "%s/path_test", TEST_LOGPATH);
...
#pragma GCC diagnostic ignored "-Wformat-truncation"
  snprintf(logfile, sizeof(logfile), "%s/path_test.log", test_path);
...
void flb_test_logrotate_mkdir(void) {
  ...
  snprintf(nested_path, sizeof(nested_path), "%s/nested/deep/path",
           TEST_LOGPATH);
/* Construct logfile path - nested_path is short so this is safe */
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wformat-truncation"
  snprintf(logfile, sizeof(logfile), "%s/test_mkdir.log", nested_path);
#pragma GCC diagnostic pop
...
void flb_test_logrotate_delimiter(void) {
  ...
  TEST_CHECK(out_ffd >= 0);
  TEST_ASSERT(flb_output_set(ctx, out_ffd, "match", "test", "file", logfile,
                             "format", "csv", "delimiter", "tab", "max_size",
                             "100M", "gzip", "false", NULL) == 0);
  ...
  /* Verify tab delimiter is used (should contain tabs, not commas) */
  content = read_file_content(logfile, &content_size);
  TEST_CHECK(content != NULL);
  if (content) {
...
void flb_test_logrotate_label_delimiter(void) {
  ...
  if (content) {
    ...
    /* LTSV format prints "time" (with quotes) followed by delimiter */
    TEST_CHECK(strstr(content, "\"time\",") != NULL);
    flb_free(content);
  }
...
static void *thread_worker(void *arg) {
  ...
  for (i = 0; i < data->events_per_thread; i++) {
    bytes =
        flb_lib_push(data->ctx, data->in_ffd, data->json_data, data->json_len);
...
void flb_test_logrotate_multithreaded(void) {
  ...
  /* Verify file content is valid - read and check for expected data */
  content = read_file_content(logfile, &content_size);
  TEST_CHECK(content != NULL);
  if (content) {
    /* Should contain tag */
...
void flb_test_logrotate_basic_rotation(void) {
  ...
  TEST_CHECK(
      count_files_in_directory(TEST_LOGPATH, "flb_test_logrotate.log.") >= 1);
...
void flb_test_logrotate_gzip_compression(void) {
  ...
  TEST_CHECK(out_ffd >= 0);
  TEST_ASSERT(flb_output_set(ctx, out_ffd, "match", "test", "file", logfile,
                             "max_size", "5K", "max_files", "3", "gzip", "true",
                             NULL) == 0);
  ...
  ret = wait_for_file_pattern(TEST_LOGPATH, "flb_test_logrotate.log.", ".gz",
                              TEST_TIMEOUT);
  TEST_CHECK(ret == 0);
...
void flb_test_logrotate_gzip_compression_exact_chunk(void) {
  ...
  snprintf(json_payload, json_size, "[%lu, {\"message\": \"%s\"}]", time(NULL),
           large_message);
