...
void flb_test_logrotate_format_template(void)
{
    ...
    /* Write some data */
    for (i = 0; i < 3; i++) {
        bytes = flb_lib_push(ctx, in_ffd, (char *)json_template, strlen(json_template));
    ...
    if (content) {
        ...
        TEST_CHECK(strstr(content, "1448403340") != NULL || strstr(content, ".") != NULL);
        flb_free(content);
    }
...
void flb_test_logrotate_mkdir(void)
{
    ...
    snprintf(nested_path, sizeof(nested_path), "%s/nested/deep/path", TEST_LOGPATH);
...
static void *thread_worker(void *arg)
{
    ...
    for (i = 0; i < data->events_per_thread; i++) {
        bytes = flb_lib_push(data->ctx, data->in_ffd, data->json_data, data->json_len);
