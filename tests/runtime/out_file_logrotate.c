...
void flb_test_file_logrotate_same_second_rotations(void);
void flb_test_file_logrotate_filename_pattern(void);
void flb_test_file_logrotate_cleanup_legacy_format(void);
...
TEST_LIST = {
    ...
    {"same_second_rotations", flb_test_file_logrotate_same_second_rotations},
    {"rotation_filename_pattern", flb_test_file_logrotate_filename_pattern},
    {"cleanup_legacy_rotation_names", flb_test_file_logrotate_cleanup_legacy_format},
    {NULL, NULL}};
...
void flb_test_file_logrotate_same_second_rotations(void)
{
    ...
    int out_ffd;
    char logfile[512];
    static const char rotated_prefix[] = "flb_test_file_logrotate.log.";
    ...
    TEST_ASSERT(flb_output_set(ctx, out_ffd, "match", "test", "file", logfile,
                               ...
                               "max_files", "10", "gzip", "false", NULL) == 0);

    ret = flb_start(ctx);
    TEST_CHECK(ret == 0);

    /*
     * Three waves of writes (cf. basic_rotation): growing past max_size,
     * then again after one rotate(), then again after a second rotate().
     */
    for (i = 0; i < 4; i++) {
        bytes = flb_lib_push(ctx, in_ffd, p, strlen(p));
        TEST_CHECK(bytes == strlen(p));
