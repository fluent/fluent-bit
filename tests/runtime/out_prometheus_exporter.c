...
static void test_http_server_options(void)
{
    ...
    /* Multi-worker HTTP servers require shared listener ports (SO_REUSEPORT),
     * which are not supported on Windows. Use a single worker there so the
     * server starts; the parsed options are still verified on both platforms.
     */
#ifdef _WIN32
    const char *workers_str = "1";
    int expected_workers = 1;
#else
    const char *workers_str = "2";
    ...
    ret = flb_output_set(ctx, out_ffd,
                         ...
                         "http_server.workers", workers_str,
                         NULL);
    TEST_CHECK(ret == 0);
    ...
    TEST_CHECK(prom_http->server.workers == expected_workers);
#ifndef _WIN32
    /* workers > 1 forces the server onto a dedicated downstream worker
     * runtime; a single worker runs on the caller event loop (runtime NULL).
     */
