...
/* Test functions */
void flb_test_in_random_flush_5s(void);

/* Test list */
TEST_LIST = {
    {"flush_5s",    flb_test_in_random_flush_5s },
    {NULL, NULL}
};
...
void flb_test_in_random_flush_5s(void)
{
    ...
    /* start test */
    sleep(2);
    ret = get_result(); /* 2sec passed, no data should be flushed */
    TEST_CHECK(ret == false);

    sleep(3 + 1);
    ret = get_result(); /* 5sec passed, data should be flushed */
    TEST_CHECK(ret == true);
