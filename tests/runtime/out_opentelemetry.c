...
void flb_test_otel_oauth2_token_source_invalid_disabled(void);
void flb_test_otel_oauth2_token_source_invalid_enabled(void);
void flb_test_otel_oauth2_metadata_smoke(void);

TEST_LIST = {
    {"default_config",         flb_test_otel_default_config},
    {"oauth2_token_source_invalid_disabled",
                               flb_test_otel_oauth2_token_source_invalid_disabled},
    {"oauth2_token_source_invalid_enabled",
                               flb_test_otel_oauth2_token_source_invalid_enabled},
...
/* An invalid oauth2.token_source must not abort plugin init while oauth2 is
 * disabled: the resolver is gated behind oauth2.enable, so the value is left
 * unparsed and the plugin starts normally. */
void flb_test_otel_oauth2_token_source_invalid_disabled(void)
{
    int ret;
    flb_ctx_t *ctx;
...
/* An invalid oauth2.token_source must still fail plugin init when oauth2 is
 * enabled, because the resolver runs inside the oauth2-enabled guard. */
void flb_test_otel_oauth2_token_source_invalid_enabled(void)
{
    int ret;
    flb_ctx_t *ctx;
...
static int smoke_mock_server_start(struct smoke_mock_server *server)
{
    ...
    /* Seed the sentinel before any fallible step so an early-failure teardown
     * never inspects an uninitialised descriptor. */
...
/* smoke_mock_server_wait_ready probes the listener until it accepts a
 * connection, so the smoke flow never races a not-yet-listening socket.
 * Returns 0 once ready, -1 if it never becomes ready within the retry budget. */
static int smoke_mock_server_wait_ready(struct smoke_mock_server *server)
{
    int                retries = 50;
    int                ret;
...
void flb_test_otel_oauth2_metadata_smoke(void)
{
    ...
    /* Block until the listener is accepting connections so the plugin's first
     * metadata fetch cannot lose a race against an unready socket. */
    ret = smoke_mock_server_wait_ready(&server);
    if (!TEST_CHECK(ret == 0)) {
        TEST_MSG("mock server never became ready");
