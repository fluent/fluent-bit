...
static void test_compression_strategy_precedence()
{
    struct flb_azure_blob ctx;
    int ret;
    int algorithm;
    ...
    ctx.compress_blob = FLB_TRUE;
    ctx.compression = FLB_COMPRESSION_ALGORITHM_ZSTD;
    ret = azb_select_compression_strategy(&ctx, &algorithm, &prefer_blob);
    TEST_CHECK(ret == 0);
    TEST_CHECK(algorithm == FLB_COMPRESSION_ALGORITHM_ZSTD);
    TEST_CHECK(prefer_blob == FLB_TRUE);

    ctx.compress_blob = FLB_TRUE;
    ctx.compression = FLB_COMPRESSION_ALGORITHM_NONE;
    ret = azb_select_compression_strategy(&ctx, &algorithm, &prefer_blob);
    TEST_CHECK(ret == 0);
    TEST_CHECK(algorithm == FLB_COMPRESSION_ALGORITHM_GZIP);
    TEST_CHECK(prefer_blob == FLB_TRUE);
    ...
    TEST_CHECK(ret == 0);
    TEST_CHECK(algorithm == FLB_COMPRESSION_ALGORITHM_ZSTD);
    TEST_CHECK(prefer_blob == FLB_FALSE);

    ctx.compress_blob = FLB_FALSE;
    ctx.compression = FLB_COMPRESSION_ALGORITHM_NONE;
    ret = azb_select_compression_strategy(&ctx, &algorithm, &prefer_blob);
    TEST_CHECK(ret == 0);
    TEST_CHECK(algorithm == FLB_COMPRESSION_ALGORITHM_NONE);
    TEST_CHECK(prefer_blob == FLB_FALSE);
...
TEST_LIST = {
    ...
    {"compression_strategy_precedence", test_compression_strategy_precedence},
    { 0 }
};
