...
void test_http_urlencoded()
{
    ...
    char *buf = "test_message=foo%5f%5Fbar";

    clear_output_num();

    cb_data.cb = cb_check_result_json;
    cb_data.data = "\"test_message\":\"foo__bar\"";
    ...
    c = flb_http_client(ctx->httpc->u_conn, FLB_HTTP_POST, "/", buf, strlen(buf),
                        "127.0.0.1", 9880, NULL, 0);
    ret = flb_http_add_header(c, FLB_HTTP_HEADER_CONTENT_TYPE, strlen(FLB_HTTP_HEADER_CONTENT_TYPE),
                              URLENCODED_CONTENT_TYPE, strlen(URLENCODED_CONTENT_TYPE));
    TEST_CHECK(ret == 0);
    if (!TEST_CHECK(c != NULL)) {
        TEST_MSG("http_client failed");
        exit(EXIT_FAILURE);
...
TEST_LIST = {
    {"http", flb_test_http},
    {"urlencoded", test_http_urlencoded},
