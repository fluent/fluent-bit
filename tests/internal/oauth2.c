...
void test_http_retry_on_401(void)
{
    int ret;
    size_t b_sent = 0;
    struct flb_config *config;
    ...
    ret = oauth2_mock_server_start(&server, 60, 1);
    TEST_CHECK(ret == 0);

    ctx = create_oauth_ctx(config, &server, 10);
    TEST_CHECK(ctx != NULL);

    u = flb_upstream_create(config, "127.0.0.1", server.port, FLB_IO_TCP, NULL);
    TEST_CHECK(u != NULL);
    ...
    client = flb_http_client(conn, FLB_HTTP_GET, "/resource", NULL, 0,
                             "127.0.0.1", server.port, NULL, 0);
    TEST_CHECK(client != NULL);

    ret = flb_http_do_with_oauth2(client, &b_sent, ctx);
    TEST_CHECK(ret == 0);
    TEST_CHECK(client->resp.status == 200);
    TEST_CHECK(server.token_requests == 2);
    TEST_CHECK(server.resource_requests == 2);
...
TEST_LIST = {
    ...
    {"http_retry_on_401", test_http_retry_on_401},
    {0}
};
