...
void cb_keys()
{
    ...
    printf("\n=== test ===");
    cra = flb_cobj_ra_create("$aaa['a'] extra $bbb['b'] final access", FLB_TRUE);
    TEST_CHECK(cra != NULL);
    if (!cra) {
        exit(EXIT_FAILURE);
    }
    TEST_CHECK(mk_list_size(&cra->list) == 4);
    flb_cobj_ra_dump(cra);
    flb_cobj_ra_destroy(cra);

    printf("\n=== test ===");
    cra = flb_cobj_ra_create("$b['x']['y']", FLB_TRUE);
    TEST_CHECK(cra != NULL);
    if (!cra) {
        exit(EXIT_FAILURE);
    }
    ...
    printf("\n=== test ===");
    cra = flb_cobj_ra_create("$z", FLB_TRUE);
    TEST_CHECK(cra != NULL);
    if (!cra) {
        exit(EXIT_FAILURE);
    }
    ...
    printf("\n=== test ===");
    cra = flb_cobj_ra_create("abc", FLB_TRUE);
    TEST_CHECK(cra != NULL);
    if (!cra) {
        exit(EXIT_FAILURE);
    }
    ...
    cra = flb_cobj_ra_create("$abc['a'", FLB_TRUE);
    TEST_CHECK(cra == NULL);

    cra = flb_cobj_ra_create("", FLB_TRUE);
    flb_cobj_ra_destroy(cra);
}
...
void cb_dash_key()
{
    ...
    /* Set up CFL object(cobj) */
    cfl_object_set(cobj, CFL_OBJECT_KVLIST, kvlist);

    /* Formatter */
    fmt = flb_sds_create("$key-dash");
    fmt_out = "something";

    cra = flb_cobj_ra_create(fmt, FLB_FALSE);
    TEST_CHECK(cra != NULL);
    if (!cra) {
        exit(EXIT_FAILURE);
    }
    ...
    /* Do translation */
    str = flb_cobj_ra_translate(cra, NULL, -1, *cobj, NULL);
    TEST_CHECK(str != NULL);
    if (!str) {
        exit(EXIT_FAILURE);
    }
...
void cb_translate()
{
    ...
    cra = flb_cobj_ra_create(fmt, FLB_TRUE);
    TEST_CHECK(cra != NULL);
    if (!cra) {
        exit(EXIT_FAILURE);
    }
...
void cb_translate_tag()
{
    ...
    /* Do translation */
    str = flb_cobj_ra_translate(cra, "testapp", 7, *cobj, NULL);
    TEST_CHECK(str != NULL);
    if (!str) {
        exit(EXIT_FAILURE);
    }
...
void cb_get_kv_pair()
{
    ...
    /* Do translation */
    ret = flb_cobj_ra_get_kv_pair(cra, *cobj, &start_key, &out_key, &out_val);
    TEST_CHECK(ret == 0);
    if (ret != 0) {
        exit(EXIT_FAILURE);
    }
...
static int order_lookup_check(struct cfl_object *cobj,
...
void cb_key_order_lookup()
{
    ...
    printf("\n-- record --\n");
    cfl_object_print(stdout, cobj);

    /* check expected outputs per record accessor pattern */
    order_lookup_check(cobj, "$bool", "true");
    order_lookup_check(cobj, "$str" , "second");
    order_lookup_check(cobj, "$num" , "1");
...
void cb_update_key_val()
{
    ...
    struct flb_cobj_record_accessor *cra = NULL;
    struct flb_cobj_record_accessor *updated_cra = NULL;
    ...
    updated_cra = flb_cobj_ra_create(updated_fmt, FLB_FALSE);
    if(!TEST_CHECK(updated_cra != NULL)) {
        exit(EXIT_FAILURE);
    }

    /* Update only value */
    ret = flb_cobj_ra_update_kv_pair(cra, *cobj, in_key, in_val);
    TEST_CHECK(ret == 0);

    ret = flb_cobj_ra_get_kv_pair(updated_cra, *cobj, &start_key, &out_key, &out_val);
    if (!TEST_CHECK(ret == 0)) {
    ...
    flb_sds_destroy(fmt);
    flb_cobj_ra_destroy(updated_cra);
    flb_cobj_ra_destroy(cra);
    cfl_object_destroy(cobj);
...
void cb_update_val()
{
    ...
    /* Update only value */
    ret = flb_cobj_ra_update_kv_pair(cra, *cobj, NULL, in_val);
    TEST_CHECK(ret == 0);
...
void cb_update_key()
{
    ...
    /* Update only value */
    ret = flb_cobj_ra_update_kv_pair(cra, *cobj, in_key, NULL);
    TEST_CHECK(ret == 0);
...
void cb_ra_translate_check()
{
    ...
    str = flb_cobj_ra_translate_check(cra, NULL, -1, *cobj, NULL, check_translation);
...
void cb_ra_create_str_from_list()
{
    ...
    for (case_i = 0; case_i < case_size; case_i++) {
        ...
        ret_str = flb_cobj_ra_create_str_from_list(list);
...
void cb_add_key_val()
{
    ...
    /* Add key/value */
    ret = flb_cobj_ra_append_kv_pair(cra, *cobj, in_val);
    TEST_CHECK(ret == 0);
