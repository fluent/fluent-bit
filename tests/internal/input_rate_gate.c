...
static void test_rate_gate_backpressure_limited()
{
    ...
    ctx.input->rate_gate_max_bytes = 1;
    ...
    TEST_CHECK(ctx.input->rate_gate_timer != NULL);
...
static void test_rate_gate_property_parsing()
{
    ...
    ret = flb_input_set_property(ctx.input, "rate_gate.max_records", "999999999999999999999");
    TEST_CHECK(ret == -1);
    TEST_CHECK(ctx.input->rate_gate_max_records == 30);

    ret = flb_input_set_property(ctx.input, "rate_gate.max_records", " -1");
    TEST_CHECK(ret == -1);
    TEST_CHECK(ctx.input->rate_gate_max_records == 30);

    ret = flb_input_set_property(ctx.input, "rate_gate.resume_ratio", "0.5junk");
    TEST_CHECK(ret == -1);
    TEST_CHECK(ctx.input->rate_gate_resume_ratio > 0.69 &&
               ctx.input->rate_gate_resume_ratio < 0.71);

    ret = flb_input_set_property(ctx.input, "rate_gate.resume_ratio", "nan");
    TEST_CHECK(ret == -1);
    TEST_CHECK(ctx.input->rate_gate_resume_ratio > 0.69 &&
               ctx.input->rate_gate_resume_ratio < 0.71);
...
static void test_rate_update_clock_rollback()
{
    int ret;
    uint64_t ts;
    struct test_context ctx;
    ...
    ts = cfl_time_now();
    ctx.input->rate_window_start = ts + FLB_NSEC_IN_SEC;
    ctx.input->rate_window_size = FLB_NSEC_IN_SEC;
    ctx.input->rate_window_records = 100;
    ctx.input->rate_window_bytes = 1000;
    ctx.input->rate_records = 100.0;
    ctx.input->rate_bytes = 1000.0;

    flb_input_rate_update(ctx.input, ts, 2, 20);

    TEST_CHECK(ctx.input->rate_window_start == ts);
    TEST_CHECK(ctx.input->rate_window_records == 2);
    TEST_CHECK(ctx.input->rate_window_bytes == 20);
    TEST_CHECK(ctx.input->rate_records == 0.0);
    TEST_CHECK(ctx.input->rate_bytes == 0.0);
...
static void test_rate_update_saturates_counters()
{
    int ret;
    uint64_t ts;
    struct test_context ctx;
    ...
    ctx.input->rate_window_start = ts;
    ctx.input->rate_window_size = FLB_NSEC_IN_SEC;
    ctx.input->rate_window_records = SIZE_MAX - 1;
    ctx.input->rate_window_bytes = SIZE_MAX - 1;

    flb_input_rate_update(ctx.input, ts, 2, 2);

    TEST_CHECK(ctx.input->rate_window_records == SIZE_MAX);
    TEST_CHECK(ctx.input->rate_window_bytes == SIZE_MAX);

    test_context_destroy(&ctx);
}

static void test_rate_update_skipped_idle_window()
{
    int ret;
    uint64_t ts;
    struct test_context ctx;
    ...
    flb_input_rate_update(ctx.input, ts + (2 * FLB_NSEC_IN_SEC), 1, 10);

    TEST_CHECK(ctx.input->rate_records == 0.0);
    TEST_CHECK(ctx.input->rate_bytes == 0.0);
    TEST_CHECK(ctx.input->rate_window_records == 1);
    TEST_CHECK(ctx.input->rate_window_bytes == 10);
    TEST_CHECK(ctx.input->rate_window_start == ts + (2 * FLB_NSEC_IN_SEC));
...
static void test_rate_gate_hysteresis_resume()
{
    ...
    ctx.input->rate_window_bytes = 1101;
    ret = flb_input_rate_gate_protect(ctx.input);
    TEST_CHECK(ret == FLB_TRUE);
    TEST_CHECK(ctx.input->rate_gate_status == FLB_INPUT_PAUSED);

    ctx.input->rate_window_bytes = 850;
    flb_input_rate_gate_maybe_resume(ctx.input);
    TEST_CHECK(ctx.input->rate_gate_status == FLB_INPUT_PAUSED);

    ctx.input->rate_window_bytes = 750;
    flb_input_rate_gate_maybe_resume(ctx.input);
    TEST_CHECK(ctx.input->rate_gate_status == FLB_INPUT_RUNNING);
    TEST_CHECK(ctx.input->rate_gate_timer == NULL);
...
static void test_rate_gate_pause_resume_stability()
{
    ...
    ctx.input->rate_window_records = 101;
    ret = flb_input_rate_gate_protect(ctx.input);
    TEST_CHECK(ret == FLB_TRUE);
    TEST_CHECK(ctx.input->rate_gate_status == FLB_INPUT_PAUSED);
    TEST_CHECK(ctx.input->rate_gate_timer != NULL);
    ...
    ctx.input->rate_window_records = 1;
    flb_input_rate_gate_maybe_resume(ctx.input);
    TEST_CHECK(ctx.input->rate_gate_status == FLB_INPUT_RUNNING);
    TEST_CHECK(ctx.input->rate_gate_timer == NULL);
...
static void test_rate_gate_resumes_after_window_rollover()
{
    int ret;
    uint64_t ts;
    struct test_context ctx;
    ...
    ctx.input->rate_window_start = ts - FLB_NSEC_IN_SEC;
    ctx.input->rate_window_size = FLB_NSEC_IN_SEC;
    ctx.input->rate_window_records = 11;

    flb_input_rate_gate_maybe_resume(ctx.input);

    TEST_CHECK(ctx.input->rate_records >= 10.0);
    TEST_CHECK(ctx.input->rate_window_records == 0);
    TEST_CHECK(ctx.input->rate_gate_status == FLB_INPUT_RUNNING);
...
static void test_rate_gate_timer_failure_keeps_input_running()
{
    int ret;
    struct test_context ctx;
    ...
    flb_sched_destroy(ctx.config->sched);
    ctx.config->sched = NULL;
...
TEST_LIST = {
    ...
    {"rate_update_clock_rollback", test_rate_update_clock_rollback},
    {"rate_update_saturates_counters", test_rate_update_saturates_counters},
    {"rate_update_skipped_idle_window", test_rate_update_skipped_idle_window},
    ...
    {"rate_gate_resumes_after_window_rollover",
     test_rate_gate_resumes_after_window_rollover},
    {"rate_gate_timer_failure_keeps_input_running",
     test_rate_gate_timer_failure_keeps_input_running},
