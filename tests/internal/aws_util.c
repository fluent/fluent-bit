...
static void test_flb_get_s3_key_uuid_test_length()
{
    flb_sds_t s3_key_format = NULL;
    struct tm day = { 0, 0, 0, 15, 7, 120};
    time_t t = mktime(&day);
    s3_key_format = flb_get_s3_key(S3_KEY_FORMAT_UUID, t, NO_TAG,
                                   TAG_DELIMITER, 0);

    int uuid_start = strlen(s3_key_format) - 8;
    char uuid_removed[uuid_start];
    strncpy(uuid_removed, s3_key_format, uuid_start);
    uuid_removed[uuid_start] = '\0';

    TEST_CHECK(strcmp(uuid_removed, S3_OBJECT_KEY_UUID) == 0);
    TEST_CHECK(strlen(s3_key_format) == strlen(S3_OBJECT_KEY_UUID) + 8);

    flb_sds_destroy(s3_key_format);
}

static void test_flb_get_s3_key_all_options_specified()
{
    flb_sds_t s3_key_format = NULL;
    struct tm day = { 0, 0, 0, 15, 7, 120};
    time_t t = mktime(&day);
    s3_key_format = flb_get_s3_key(S3_KEY_FORMAT_ALL_OPTIONS, t, TAG,
                                   TAG_DELIMITER, 0);
    ...
    TEST_CHECK(strcmp(uuid_removed, S3_OBJECT_KEY_ALL_OPTIONS) == 0);
    TEST_CHECK(strlen(s3_key_format) == strlen(S3_OBJECT_KEY_ALL_OPTIONS) + 8);
...
TEST_LIST = {
    ...
    {"flb_get_s3_key_uuid_test_length", test_flb_get_s3_key_uuid_test_length},
    {"flb_get_s3_key_all_options_specified", test_flb_get_s3_key_all_options_specified},
