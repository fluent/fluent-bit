        ...
        struct oauth2_metadata_mock_server *server)
{
    ...
    /* Seed the listen socket sentinel before any fallible step so an
     * early-failure teardown never operates on uninitialised state. */
...
void test_token_source_parse_client_credentials(void)
{
    ...
    ret = flb_oauth2_token_source_parse("client_credentials", &out);
    TEST_CHECK(ret == 0);
...
void test_token_source_parse_metadata(void)
{
    ...
    ret = flb_oauth2_token_source_parse("metadata", &out);
    TEST_CHECK(ret == 0);
...
void test_metadata_split_header_simple(void)
{
    ...
    ret = flb_oauth2_metadata_split_header("X-Custom: value", &name, &value);
    TEST_CHECK(ret == 0);
...
void test_metadata_split_header_extra_whitespace(void)
{
    ...
    ret = flb_oauth2_metadata_split_header(
...
void test_metadata_split_header_missing_colon(void)
{
    ...
    ret = flb_oauth2_metadata_split_header("NoColonHere", &name, &value);
    TEST_CHECK(ret == -1);
    TEST_CHECK(name == NULL);
    TEST_CHECK(value == NULL);
}
...
void test_metadata_split_header_empty_value(void)
{
    ...
    ret = flb_oauth2_metadata_split_header("X-Custom:    ", &name, &value);
    TEST_CHECK(ret == -1);
    TEST_CHECK(name == NULL);
    TEST_CHECK(value == NULL);
}
...
void test_metadata_split_header_injection_rejected(void)
{
    ...
    ret = flb_oauth2_metadata_split_header("Bad\nName: value", &name, &value);
    TEST_CHECK(ret == -1);
    TEST_CHECK(name == NULL);
    TEST_CHECK(value == NULL);

    /* A non-token byte in the field-name (here a space) must be rejected. */
    name = (flb_sds_t) 0xdeadbeef;
    value = (flb_sds_t) 0xdeadbeef;
    ret = flb_oauth2_metadata_split_header("Bad Name: value", &name, &value);
    TEST_CHECK(ret == -1);
    TEST_CHECK(name == NULL);
    TEST_CHECK(value == NULL);

    /* A control byte other than CR/LF in the value must be rejected. */
    name = (flb_sds_t) 0xdeadbeef;
    value = (flb_sds_t) 0xdeadbeef;
    ret = flb_oauth2_metadata_split_header("X-Custom: va\x01lue", &name, &value);
    TEST_CHECK(ret == -1);
    TEST_CHECK(name == NULL);
    TEST_CHECK(value == NULL);
}
...
/*
 * Drive flb_oauth2_metadata_refresh_locked against the fragmenting mock with each
                                                    ...
                                                    int fragment_delay_us)
{
     ...
     */
    ret = flb_oauth2_metadata_refresh_locked(ctx);
    TEST_CHECK(ret == 0);
