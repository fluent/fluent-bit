...
struct record_check go_fatal_output[] = {
    {
...
struct record_check go_fatal_input[] = {
    {"fatal error: concurrent map writes\n"},
    {"\n"},
    {"goroutine 4 [running]:\n"},
    {"panic(0x45cb40, 0x47ad70)\n"},
...
static void test_parser_go_fatal()
{
    int i;
    int len;
    int ret;
    ...
    /* Expected results context */
    res.key = "log";
    res.out_records = go_fatal_output;
    ...
    entries = sizeof(go_fatal_input) / sizeof(struct record_check);
    for (i = 0; i < entries; i++) {
        r = &go_fatal_input[i];
        len = strlen(r->buf);
...
TEST_LIST = {
    ...
    { "parser_go_fatal", test_parser_go_fatal},
