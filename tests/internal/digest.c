...
static void test_digest_standard()
{
    ...
    for (index = 0 ; index < buffer_count ; index++) {
        result = flb_digest_update(&digest, buffer_list[index], length_list[index]);
        TEST_CHECK(result == FLB_CRYPTO_SUCCESS);
    }

    result = flb_digest_finalize(&digest, raw_digest, sizeof(raw_digest));
    TEST_CHECK(result == FLB_CRYPTO_SUCCESS);
