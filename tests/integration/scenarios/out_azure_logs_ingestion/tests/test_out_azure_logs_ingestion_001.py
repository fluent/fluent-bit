...
def test_out_azure_logs_ingestion_flushes_partial_batch_after_timeout(chunk_count):
    service = Service(
        "out_azure_logs_ingestion_batching.yaml",
    ...
    try:
        ...
        # The configured timeout is three seconds. In particular, Linux must
        # not treat each nominal subsecond callback timer as one elapsed
        # interval and finish a three-second wait in a few milliseconds.
        before_deadline = 0.75 - (time.monotonic() - timeout_started)
        if before_deadline > 0:
            time.sleep(before_deadline)
            assert data_requests(data_storage["requests"]) == []

        requests_seen = service.wait_for_requests(2, timeout=10)
        elapsed = time.monotonic() - timeout_started
        assert 2.5 <= elapsed < 8
    finally:
        service.stop()
