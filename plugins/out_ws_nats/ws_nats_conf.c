...
struct flb_out_ws_nats *flb_ws_nats_conf_create(struct flb_output_instance *ins,
                                          struct flb_config *config)
{
    ...
    ctx = flb_calloc(1, sizeof(struct flb_out_ws_nats));
    if (!ctx) {
        flb_errno();
        return NULL;
    }
    ...
    if (tmp) {
        ...
        if (ret == -1) {
            flb_error("[out_ws_nats] unrecognized 'format' option '%s'. Using 'msgpack'", tmp);
        }
        else {
            ctx->out_format = ret;
        }
    ...
    if (tmp) {
        ...
        if (ret == -1) {
            flb_error("[out_ws_nats] unrecognized 'json_date_format' option '%s'. Using 'double'", tmp);
        }
        else {
    ...
    if (idle_interval > 5) {
        ...
        flb_error("[out_ws_nats] the keepalive timeout value is smaller than 2, which is meaningless! Please set it higher than 10 seconds. Current value will bring disorder for websocket plugin.");
    ...
    flb_info("[out_ws_nats] we have following parameter %s, %s, %d, %d", ctx->uri, ctx->host, ctx->port, ctx->idle_interval);
    return ctx;
}

void flb_ws_nats_conf_destroy(struct flb_out_ws_nats *ctx)
{
    flb_info("[out_ws_nats] flb_ws_nats_conf_destroy ");
    if (!ctx) {
        return;
    }
