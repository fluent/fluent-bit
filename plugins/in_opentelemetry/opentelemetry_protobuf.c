...
struct protobuf_frame {
    const ProtobufCMessageDescriptor *descriptor;
    const unsigned char *cursor;
    const unsigned char *end;
};

static int read_varint(struct protobuf_frame *frame, uint64_t *value)
{
    unsigned int shift;
    unsigned char byte;
 ...
 */
int opentelemetry_protobuf_validate(const ProtobufCMessageDescriptor *descriptor,
                                    const void *data, size_t size)
{
    struct protobuf_frame frames[OTEL_PROTOBUF_MAX_DEPTH];
    struct protobuf_frame *frame;
    ...
    while (depth > 0) {
        ...
        if (read_varint(frame, &tag) != 0 || tag >> 3 == 0 || tag >> 3 > 0x1fffffff) {
            return -1;
        }

        switch (tag & 7) {
        case 0:
            if (read_varint(frame, &length) != 0) {
                return -1;
            }
            continue;
        case 1:
        ...
        if (depth >= OTEL_PROTOBUF_MAX_DEPTH) {
            return -1;
        }
