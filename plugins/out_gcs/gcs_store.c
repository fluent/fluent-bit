...
static int gcs_store_under_travis_ci()
{
...
/* Retrieve a candidate gcs local file using the tag */
                        ...
                        char *data, size_t bytes)
{
    ...
    if (!gcs_file) {
        ...
        gcs_file = flb_calloc(1, sizeof(struct gcs_file));
        if (!gcs_file) {
            flb_errno();
            flb_plg_error(ctx->ins, "cannot allocate gcs file context");
            flb_plg_warn(ctx->ins, "Deleting buffer file because GCS context creation failed");
            flb_fstore_file_delete(ctx->fs, fsf);
            return -1;
        }
        gcs_file->fsf = fsf;
        gcs_file->create_time = time(NULL);
    ...
    if (ret != 0) {
        flb_plg_error(ctx->ins, "error writing data to local gcs file");
        return -1;
    }
    gcs_file->size += bytes;
...
static int set_files_context(struct flb_gcs *ctx)
{
    struct mk_list *head;
...
/* Initialize filesystem storage for GCS plugin */
int gcs_store_init(struct flb_gcs *ctx)
{
    ...
    if (gcs_store_under_travis_ci() == FLB_TRUE) {
        type = FLB_FSTORE_MEM;
        flb_plg_warn(ctx->ins, "Travis CI test, using gcs store memory backend");
    }
    else {
        type = FLB_FSTORE_FS;
    }
     ...
     * considered backlog data, in the GCS plugin we need to differenciate the
...
int gcs_store_exit(struct flb_gcs *ctx)
{
    ...
    mk_list_foreach(head, &ctx->fs->streams) {
        ...
        mk_list_foreach(f_head, &fs_stream->files) {
            ...
            if (fsf->data != NULL) {
                gcs_file = fsf->data;
                flb_sds_destroy(gcs_file->file_path);
                flb_free(gcs_file);
            }
        }
    }
...
int gcs_store_file_delete(struct flb_gcs *ctx, struct gcs_file *gcs_file)
{
    ...
    fsf = gcs_file->fsf;
    ctx->current_buffer_size -= gcs_file->size;
                               ...
                               char **out_buf, size_t *out_size)
{
    ...
    ret = flb_fstore_file_content_copy(ctx->fs, gcs_file->fsf,
