...
static inline int grep_filter_metrics_or_op(struct cmt *cmt, struct cmt *out_cmt, struct grep_ctx *ctx)
{
    ssize_t ret;
    int found = FLB_FALSE;
    struct mk_list *head;
...
static inline int grep_filter_metrics_and_op(struct cmt *cmt, struct cmt *out_cmt, struct grep_ctx *ctx)
{
    int ret;
    int found = FLB_FALSE;
    struct mk_list *head;
