                                 ...
                                 struct flb_config *config)
{
    ...
    if (f_index) {
        flb_es_str_copy_str(&ec->index, f_index->value);
        if (f_index->value && !ec->index.value) {
    ...
    if (f_type) {
        flb_es_str_copy_str(&ec->type, f_type->value);
        if (f_type->value && !ec->type.value) {
                                      ...
                                      struct flb_config *config)
{
    ...
    if (tmp) {
        if (flb_es_sds_copy_str(&ec->logstash_prefix, tmp) == NULL) {
            return -1;
        }
    }
    tmp = flb_upstream_node_get_property(FLB_ES_CONFIG_PROPERTY_LOGSTASH_PREFIX_SEPARATOR,
                                         node);
    if (tmp) {
        if (flb_es_sds_copy_str(&ec->logstash_prefix_separator, tmp) == NULL) {
            return -1;
        }
    }
    ...
    if (tmp) {
        if (flb_es_sds_copy_str(&ec->tag_key, tmp) == NULL) {
            return -1;
        }
    }
    ...
    tmp = flb_upstream_node_get_property(FLB_ES_CONFIG_PROPERTY_LOGSTASH_PREFIX_KEY,
                                         node);
    if (tmp) {
        if (flb_es_sds_copy_str(&ec->logstash_prefix_key, tmp) == NULL) {
            return -1;
        }
        ret = config_set_ra_prefix_key(ec->logstash_prefix_key.value, ec, ctx);
        if (ret != 0) {
    ...
    if (tmp) {
        if (flb_es_sds_copy_str(&ec->id_key, tmp) == NULL) {
            return -1;
        }
        ret = config_set_ra_id_key(ec->id_key.value, ec, ctx);
        if (ret != 0) {
    ...
    if (tmp) {
        if (flb_es_sds_copy_str(&ec->write_operation, tmp) == NULL) {
            return -1;
        }
    }
