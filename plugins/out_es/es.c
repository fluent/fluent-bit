                                           ...
                                           struct flb_elasticsearch *ctx)
{
     ...
     * Lifetime strategy:
     *
     * This helper always returns a borrowed view over memory owned by either:
     *
     * - the upstream node property table, or
     * - the output instance property table.
     *
     * The returned view must be consumed within the current flush/config call
     * path and must never be stored across calls or freed by the caller.
     *
     * Precedence strategy:
...
static const char *es_get_action_from_write_operation(flb_sds_view_t write_operation,
                                                      const char *base_action)
{
    ...
    if (flb_sds_casecmp(write_operation.buf, FLB_ES_WRITE_OP_INDEX,
                        write_operation.len) == 0) {
        return FLB_ES_WRITE_OP_INDEX;
    }

    if (flb_sds_casecmp(write_operation.buf, FLB_ES_WRITE_OP_CREATE,
                        write_operation.len) == 0) {
        return FLB_ES_WRITE_OP_CREATE;
    }

    if (flb_sds_casecmp(write_operation.buf, FLB_ES_WRITE_OP_UPDATE,
                        write_operation.len) == 0 ||
        flb_sds_casecmp(write_operation.buf, FLB_ES_WRITE_OP_UPSERT,
                        write_operation.len) == 0) {
                                ...
                                msgpack_object *map)
{
    ...
    if (rval == NULL) {
        flb_plg_warn(ctx->ins, "the value of %.*s is missing",
                     (int) id_key_name.len, id_key_name.buf);
        return NULL;
    }
...
static int compose_index_header(int es_index_custom_len,
                                ...
                                flb_sds_view_t logstash_prefix,
                                flb_sds_view_t separator,
                                flb_sds_view_t dateformat,
                                struct tm *tm)
{
    int ret;
    int len;
    ...
    if (es_index_custom_len > 0) {
        ...
        p = logstash_index + logstash_prefix.len;
    }
    len = p - logstash_index;
    ret = snprintf(p, logstash_index_size - len, "%.*s",
                   (int) separator.len, separator.buf);
                                ...
                                void **out_data, size_t *out_size)
{
    ...
    flb_sds_view_t id_key_name;
    flb_sds_view_t write_operation;
    flb_sds_view_t logstash_prefix;
    flb_sds_view_t logstash_prefix_separator;
    flb_sds_view_t logstash_dateformat;
    flb_sds_view_t type_name;
    ...
    write_operation = es_get_property_view("write_operation", node, ctx);
    ...
    if (!flb_sds_view_is_empty(write_operation)) {
        ...
        else if (flb_sds_casecmp(write_operation.buf, FLB_ES_WRITE_OP_UPSERT,
                                 write_operation.len) == 0) {
    ...
    id_key_name = es_get_property_view("id_key", node, ctx);
    if (flb_sds_view_is_empty(id_key_name) && ctx->id_key != NULL) {
        id_key_name = flb_sds_view_create(ctx->id_key, flb_sds_len(ctx->id_key));
    }

    logstash_prefix = es_get_property_view("logstash_prefix", node, ctx);
    if (flb_sds_view_is_empty(logstash_prefix) && ctx->logstash_prefix != NULL) {
        logstash_prefix = flb_sds_view_create(ctx->logstash_prefix,
                                              flb_sds_len(ctx->logstash_prefix));
    }

    logstash_prefix_separator = es_get_property_view("logstash_prefix_separator",
                                                     node, ctx);
    if (flb_sds_view_is_empty(logstash_prefix_separator) &&
        ctx->logstash_prefix_separator != NULL) {
        logstash_prefix_separator = flb_sds_view_create(ctx->logstash_prefix_separator,
                                                        flb_sds_len(ctx->logstash_prefix_separator));
    }

    logstash_dateformat = es_get_property_view("logstash_dateformat", node, ctx);
    if (flb_sds_view_is_empty(logstash_dateformat) &&
        ctx->logstash_dateformat != NULL) {
        logstash_dateformat = flb_sds_view_create(ctx->logstash_dateformat,
                                                  flb_sds_len(ctx->logstash_dateformat));
    }

    /* Copy logstash prefix if logstash format is enabled */
    if (logstash_format == FLB_TRUE) {
        strncpy(logstash_index, logstash_prefix.buf, sizeof(logstash_index));
    ...
    type_name = es_get_property_view("type", node, ctx);
    if (flb_sds_view_is_empty(type_name) && ctx->type != NULL) {
        type_name = flb_sds_view_create(ctx->type, strlen(ctx->type));
    }
        ...
        if (generate_id == FLB_TRUE) {
            ...
            if (suppress_type_name == FLB_TRUE) {
                ...
                index_len = flb_sds_snprintf(&j_index,
                                             ...
                                             es_action,
                                             es_index, type_name.buf, es_uuid);
            }
        }
        if (ra_id_key != NULL) {
            ...
            if (id_key_str) {
                if (suppress_type_name == FLB_TRUE) {
                    ...
                    index_len = flb_sds_snprintf(&j_index,
                                                 ...
                                                 ES_BULK_INDEX_FMT_ID,
                                                 es_action,
                                                 es_index, type_name.buf, id_key_str);
                }
