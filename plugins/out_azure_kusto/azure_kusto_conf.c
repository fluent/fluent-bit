                                         ...
                                         struct flb_config *config)
{
    ...
    if (ctx->resources->blob_ha && ctx->resources->queue_ha &&
        ...
        now - ctx->resources->load_time < ctx -> ingestion_resources_refresh_interval) {
        flb_plg_debug(ctx->ins, "resources are already loaded and are not stale");
        ret = 0;
    }
    else {
        flb_plg_info(ctx->ins, "loading kusto ingestion resourcs and refresh interval is %d", ctx -> ingestion_resources_refresh_interval);
