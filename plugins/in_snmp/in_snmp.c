...
int mock_snmp_synch_response(netsnmp_session *ss, netsnmp_pdu *pdu,
                              netsnmp_pdu **response)
{
    ...
    if (strcmp(mock_status, "snmp_get") == 0) {
        mock_netsnmp_variable_list = flb_calloc(1,
                                                sizeof(netsnmp_variable_list));
        if (mock_netsnmp_variable_list == NULL) {
            goto error;
        }

        /* sysUpTime */
        mock_objid_len = MAX_OID_LEN;
        if (snmp_parse_oid("1.3.6.1.2.1.1.3.0", mock_objid,
                           &mock_objid_len) == NULL) {
            goto error;
        }

        if (snmp_set_var_objid(mock_netsnmp_variable_list, mock_objid,
                               mock_objid_len)) {
            goto error;
        }

        if (snmp_set_var_typed_integer(mock_netsnmp_variable_list,
                                        ASN_TIMETICKS, 123)) {
            goto error;
        }
        ...
        *response = mock_response;
        ...
        /* ifName.1 */
        mock_objid_len = MAX_OID_LEN;
        if (snmp_parse_oid("1.3.6.1.2.1.31.1.1.1.1.1", mock_objid,
                           &mock_objid_len) == NULL) {
            goto error;
        }
        ...
        if (snmp_set_var_typed_value(mock_netsnmp_variable_list,
                                      ASN_OCTET_STR, "Fa0/0",
                                      strlen("Fa0/0"))) {
            goto error;
        }

        mock_netsnmp_variable_list->next_variable = flb_calloc(1,
                                                    sizeof(netsnmp_variable_list));
        if (mock_netsnmp_variable_list->next_variable == NULL) {
            goto error;
        }

        /* ifName.2 */
        mock_objid_len = MAX_OID_LEN;
        if (snmp_parse_oid("1.3.6.1.2.1.31.1.1.1.1.2", mock_objid,
                           &mock_objid_len) == NULL) {
            goto error;
        }

        if (snmp_set_var_objid(mock_netsnmp_variable_list->next_variable,
                               mock_objid, mock_objid_len)) {
            goto error;
        }

        if (snmp_set_var_typed_value(mock_netsnmp_variable_list->next_variable,
                                      ASN_OCTET_STR, "Fa0/1",
                                      strlen("Fa0/1"))) {
            goto error;
        }
...
static int in_snmp_collect(struct flb_input_instance *ins,
                           struct flb_config *config, void *in_context)
{
    ...
    while (running) {
        ...
        if (status == STAT_SUCCESS && response->errstat == SNMP_ERR_NOERROR) {
            for (vars = response->variables; vars; vars = vars->next_variable) {
                if (is_walk && snmp_oid_compare(end_oid, end_len, vars->name,
                                                vars->name_length) <= 0) {
                ...
                if ((oid_buf = flb_calloc(oid_buf_len, 1)) == NULL) {
                    ...
                    netsnmp_sprint_realloc_objid_tree(&oid_buf, &oid_buf_len,
                                                      &oid_out_len, 1,
                                                      &buf_overflow, vars->name,
                                                      vars->name_length);
                    if (buf_overflow) {
                        flb_plg_error(ctx->ins, "[TRUNCATED]");
                        if (response) {
                ...
                if ((vars->type != SNMP_ENDOFMIBVIEW) &&
                    ...
                    if (snmp_oid_compare(name, name_len, vars->name,
            ...
            if (status == STAT_SUCCESS) {
                flb_plg_error(ctx->ins, "Error in packet. Reason: %s",
                              snmp_errstring(response->errstat));
                running = 0;
            }
            else if (status == STAT_TIMEOUT) {
                flb_plg_error(ctx->ins, "Timeout: No response from %s",
    ...
    if (ret == FLB_EVENT_ENCODER_SUCCESS) {
        flb_input_log_append(ins, NULL, 0, ctx->log_encoder.output_buffer,
                        ...
                        struct flb_config *config, void *data)
{
    ...
    tm.tv_sec = 1;
    tm.tv_nsec = 0;
    ret = flb_input_set_collector_time(in, in_snmp_collect, tm.tv_sec,
                                       tm.tv_nsec, config);
    if (ret < 0) {
        flb_plg_error(ctx->ins, "could not set collector for snmp plugin");
        return -1;
    }
    ...
    netsnmp_ds_set_int(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_TIMEOUT,
                       ctx->timeout);
    ...
    /* compose peername by using target_host and target_port */
    {
        ...
        int written = snprintf(peer, need, "udp:%s:%d", ctx->target_host,
                               ctx->target_port);
...
static struct flb_config_map config_map[] = {
   ...
   },
   {
    ...
    "set the target host port to collect metrics through SNMP."
   },
   {
   ...
   },
   {
    ...
    "set the type of SNMP request: 'get' for a single OID,"
    "'walk' to traverse an OID subtree."
   },
   {
