...
#define GZIP_CHUNK_SIZE                                                        \
  (16 * 1024) /* 16KB chunks to prevent stack overflow in coroutines */
...
struct logrotate_file_size {
  flb_sds_t filename;
  size_t size;
  flb_lock_t lock; /* Mutex to protect file operations */
...
static int cb_logrotate_init(struct flb_output_instance *ins,
  ...
  flb_plg_info(ctx->ins,
               "logrotate plugin initialized with: max_size=%zu, max_files=%d, "
               "gzip=%s, path=%s",
...
static int csv_output(FILE *fp, int column_names, struct flb_time *tm,
                      msgpack_object *obj, struct flb_logrotate_conf *ctx) {
  int i;
  int map_size;
...
static int ltsv_output(FILE *fp, struct flb_time *tm, msgpack_object *obj,
  ...
  if (obj->type == MSGPACK_OBJECT_MAP && obj->via.map.size > 0) {
    kv = obj->via.map.ptr;
    map_size = obj->via.map.size;
    fprintf(fp, "\"time\"%s%f%s", ctx->label_delimiter, flb_time_to_double(tm),
            ctx->delimiter);
...
static int template_output_write(struct flb_logrotate_conf *ctx, FILE *fp,
...
static int mkpath(struct flb_output_instance *ins, const char *dir) {
  ...
  if (!CreateDirectoryA(dir, NULL)) {
    ...
    if (err != ERROR_ALREADY_EXISTS) {
      flb_plg_error(ins, "could not create directory '%s' (error=%lu)", dir,
                    err);
      return -1;
    }
  }
...
static struct logrotate_file_size *
find_file_size_entry(struct flb_logrotate_conf *ctx, const char *filename) {
  struct mk_list *head;
...
static struct logrotate_file_size *
create_file_size_entry(struct flb_logrotate_conf *ctx, const char *filename,
                       size_t size) {
  struct logrotate_file_size *entry;
...
static int gzip_compress_file(const char *input_filename,
  ...
  /*
   * If the file size is a multiple of GZIP_CHUNK_SIZE, the loop above finishes
   * because bytes_read == 0, but Z_FINISH was never called (flush was
   * Z_NO_FLUSH). We must ensure the stream is finished.
   */
  if (status != Z_STREAM_END) {
    ...
    do {
      ...
      if (compressed_bytes > 0) {
        if (fwrite(output_buffer, 1, compressed_bytes, dst_fp) !=
            compressed_bytes) {
          flb_plg_error(ins, "failed to write compressed data (final flush)");
          ret = -1;
          goto deflate_cleanup;
        }
      }
 ...
 */
static int is_valid_rotation_filename(const char *filename,
                                      const char *pattern) {
...
static int cleanup_old_files(struct flb_logrotate_conf *ctx,
                             const char *directory, const char *base_filename) {
  ...
  do {
    ...
    if (is_valid_rotation_filename(findData.cFileName, pattern) &&
        i < file_count) {
      snprintf(full_path, PATH_MAX - 1, "%s" FLB_PATH_SEPARATOR "%s", directory,
               findData.cFileName);
...
static void cb_logrotate_flush(struct flb_event_chunk *event_chunk,
  ...
  struct logrotate_file_size *entry = NULL;
  struct stat st;
  ...
  /* Initialize file size counter if this is a new file (logic moved to unified
   * block above) */
  /* If we are here, we are guaranteed to have an entry and hold the lock */
  if (entry->size == 0) {
  ...
  if (ctx->format == FLB_OUT_LOGROTATE_FMT_MSGPACK) {
    ...
    do {
      ret = fwrite((char *)event_chunk->data + off, 1, event_chunk->size - off,
                   fp);
      if (ret < 0) {
        flb_errno();
        fclose(fp);
  ...
  while ((ret = flb_log_event_decoder_next(&log_decoder, &log_event)) ==
    ...
    case FLB_OUT_LOGROTATE_FMT_JSON:
      ...
      if (buf) {
        fprintf(fp, "%s: [%" PRIu64 ".%09lu, %s]" NEWLINE, event_chunk->tag,
