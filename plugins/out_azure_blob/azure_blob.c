...
/* Simple UUID replacement - replaces $UUID with 8 random chars */
static flb_sds_t azb_replace_uuid(flb_sds_t path)
{
    ...
    p = strstr(path, AZB_UUID_PLACEHOLDER);
    if (!p) {
        return path;
    }
    ...
    suffix_len = flb_sds_len(path) - prefix_len - strlen(AZB_UUID_PLACEHOLDER);

    result = flb_sds_create_size(prefix_len + 8 + suffix_len + 1);
    if (!result) {
        flb_errno();
    ...
    memcpy(result + prefix_len, random_buf, 8);
    if (suffix_len > 0) {
        memcpy(result + prefix_len + 8, p + strlen(AZB_UUID_PLACEHOLDER), suffix_len);
    }
    result[prefix_len + 8 + suffix_len] = '\0';
    flb_sds_len_set(result, prefix_len + 8 + suffix_len);

    flb_sds_destroy(path);
    return result;
}

/* Simple token replacement helper */
