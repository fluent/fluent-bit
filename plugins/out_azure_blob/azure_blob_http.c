    ...
    if (flb_oauth2_token_expired(ctx->o) == FLB_TRUE) {
        switch (ctx->atype) {
            ...
            case FLB_AZURE_AUTH_MANAGED_IDENTITY_USER:
                if (!flb_azure_msi_token_get(ctx->o)) {
                    flb_plg_error(ctx->ins, "error retrieving oauth2 access token "
                                  "(MSI access token is NULL)");
                    ret = -1;
                }
                break;
            ...
            case FLB_AZURE_AUTH_SERVICE_PRINCIPAL:
                ...
                if (!flb_oauth2_token_get(ctx->o)) {
                    flb_plg_error(ctx->ins, "error retrieving oauth2 access token - "
                                  "check Fluent Bit logs for '[oauth2]' errors "
                                  "(common causes: connection failure to '%s', "
                                  "invalid credentials, or malformed response)",
                                  ctx->oauth_url ? ctx->oauth_url : "unknown");
                    ret = -1;
                }
                else {
    ...
    if (ret == 0 && ctx->o->access_token) {
        ...
        if (!output) {
            ...
            if (pthread_mutex_unlock(&ctx->token_mutex)) {
                flb_plg_error(ctx->ins, "error unlocking token mutex");
            }
            return NULL;
        }
