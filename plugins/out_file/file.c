                       ...
                       struct file_file_size *entry)
{
    ...
    /* Generate unique suffix (timestamp + random hex tail) */
    generate_rotation_suffix(timestamp, sizeof(timestamp));
                          ...
                          struct flb_config *config)
{
    ...
    if (ctx->format == FLB_OUT_FILE_FMT_MSGPACK) {
        ...
        while (off < event_chunk->size) {
            bytes_written = fwrite((char *)event_chunk->data + off, 1,
                                   event_chunk->size - off, fp);
            if (bytes_written == 0) {
                if (ferror(fp)) {
                    flb_errno();
                    flb_plg_error(ctx->ins, "msgpack write error on %s",
                                  out_file);
                }
                else {
                    flb_plg_warn(ctx->ins,
                                 "msgpack write made no progress on %s",
                                 out_file);
                }
                clearerr(fp);
                fclose(fp);
