                          ...
                          struct flb_config *config)
{
    ...
    /* Allocate buffers on heap to avoid stack overflow with sanitizers */
    out_file = flb_malloc(PATH_MAX * 2);
    if (!out_file) {
        flb_errno();
        FLB_OUTPUT_RETURN(FLB_ERROR);
    }

    sanitized_tag = flb_malloc(PATH_MAX);
    if (!sanitized_tag) {
        flb_errno();
        flb_free(out_file);
        FLB_OUTPUT_RETURN(FLB_ERROR);
    ...
    if (ctx->out_file == NULL) {
        ret = sanitize_tag_name(event_chunk->tag, sanitized_tag, PATH_MAX);
    ...
    if (ctx->out_path) {
        if (ctx->out_file) {
            snprintf(out_file, PATH_MAX * 2, "%s" FLB_PATH_SEPARATOR "%s",
    ...
    if (event_chunk->type == FLB_INPUT_METRICS) {
        ...
        /* Only fsync when rotation is enabled - needed for accurate size
         * tracking */
        if (ctx->files_rotation == FLB_TRUE && ctx->max_size > 0) {
            fsync(fileno(fp));
        }
    ...
    fflush(fp);
#ifndef FLB_SYSTEM_WINDOWS
    /* Only fsync when rotation is enabled - needed for accurate size tracking
     */
    if (ctx->files_rotation == FLB_TRUE && ctx->max_size > 0) {
        fsync(fileno(fp));
    }
