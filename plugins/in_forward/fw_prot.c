                               ...
                               size_t *out_len)
{
    ...
    ctx = conn->ctx;
    read_limit = size;
    if (ctx->buffer_max_size < read_limit) {
        read_limit = ctx->buffer_max_size;
    }

    msgpack_unpacked_init(&result);
    while (1) {
        if (conn->buf_len >= read_limit) {
            goto error;
        }

        if (conn->buf_len >= conn->buf_size) {
            if (ctx->buffer_chunk_size > read_limit - conn->buf_size) {
                new_size = read_limit;
            }
            else {
                new_size = conn->buf_size + ctx->buffer_chunk_size;
            }

            tmp = flb_realloc(conn->buf, new_size);
            if (tmp == NULL) {
                flb_errno();
                goto error;
            }
        ...
        avail = read_limit - conn->buf_len;
        if (conn->buf_size - conn->buf_len < avail) {
            avail = conn->buf_size - conn->buf_len;
        }
