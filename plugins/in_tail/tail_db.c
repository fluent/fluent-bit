                                   ...
                                   struct flb_config *config)
{
    ...
    ret = db_apply_migration_if_needed(ctx, db, SQL_ALTER_FILES_ADD_STREAM);
    if (ret != FLB_OK) {
        flb_sqldb_close(db);
        return NULL;
    }
...
static int db_file_exists(struct flb_tail_file *file,
                          struct flb_tail_config *ctx,
                          uint64_t *id, uint64_t *inode, int64_t *offset,
                          uint64_t *offset_marker, size_t *offset_marker_size,
                          uint64_t *skip, int64_t *anchor, uint64_t *stream)
{
    int ret;
    ...
    if (ret == SQLITE_ROW) {
        ...
        *anchor = sqlite3_column_int64(ctx->stmt_get_file, 7);
        ...
        /* stream: column 8 */
        *stream = sqlite3_column_int64(ctx->stmt_get_file, 8);
...

{
    ...
    sqlite3_bind_int64(ctx->stmt_insert_file, 9, file->stream_offset);
                            ...
                            struct flb_tail_config *ctx)
{
    ...
    sqlite3_bind_int64(ctx->stmt_offset, 6, file->stream_offset);
    sqlite3_bind_int64(ctx->stmt_offset, 7, file->db_id);
