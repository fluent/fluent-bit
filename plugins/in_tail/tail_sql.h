...
#define SQL_INSERT_FILE                                             \
    ...
    "   skip, anchor, stream)" \
    "  VALUES (@name, @offset, @inode, @created, @offset_marker, " \
    "          @offset_marker_size, @skip, @anchor, @stream);"
...
#define SQL_UPDATE_OFFSET                                   \
    ...
    "skip=@skip, anchor=@anchor, stream=@stream WHERE id=@id;"
...
#define SQL_ALTER_FILES_ADD_STREAM                                  \
    "ALTER TABLE in_tail_files ADD COLUMN stream INTEGER DEFAULT 0;"
