...
#ifndef FLB_WINDOW_TAG_H
#define FLB_WINDOW_TAG_H

#include <fluent-bit/flb_hash_table.h>

#define FLB_WINDOW_TAG_HASH_MAX_ENTRIES 100000

struct throttle_tag_pane
{
    long timestamp;
    unsigned long size;
};

struct throttle_tag_window
{
    char *name;
    unsigned size;
    unsigned long total;
    ...
    int tail;
    int startup_passed;
    struct throttle_tag_pane *table;
};

struct throttle_tag_table
{
    struct flb_hash_table *windows;
    void *lock;
};

struct throttle_tag_table *create_throttle_tag_table();
...
inline static void add_new_pane(struct throttle_tag_window *stw,
                                long timestamp)
{
...
inline static void add_load(struct throttle_tag_window *stw,
                            unsigned long load)
{
...
inline static void free_stw_content(struct throttle_tag_window *stw)
{
    flb_free(stw->name);
    flb_free(stw->table);
}

inline static void free_stw(struct throttle_tag_window *stw)
{
    free_stw_content(stw);
    flb_free(stw);
}

inline static struct throttle_tag_window *find_throttle_tag_window(struct
                                                                     throttle_tag_table
                                                                     *table,
                                                                     const char
                                                                     *name,
                                                                     ...
                                                                     name_length)
{
    ...
    if (flb_hash_table_get(table->windows, name, name_length,
                           (void **)&window, &out_size) >= 0) {
        if (out_size < sizeof(struct throttle_tag_window)) {
            flb_error("Malformed data in size window hashtable");
            return NULL;
        }
        return (struct throttle_tag_window *) window;
    }
    return NULL;
}

inline static void add_throttle_tag_window(struct throttle_tag_table
                                            *table,
                                            struct throttle_tag_window
                                            *window)
{
    flb_hash_table_add(table->windows, window->name, strlen(window->name),
                       (char *) window, sizeof(struct throttle_tag_window));
}

void destroy_throttle_tag_table(struct throttle_tag_table *table);

void lock_throttle_tag_table(struct throttle_tag_table *ht);
void unlock_throttle_tag_table(struct throttle_tag_table *ht);
