...
static void
stan_connection_lost(stanConnection *sc, const char *errTxt, void *closure)
{
...
static void
nats_connection_lost(natsConnection *nc, void *closure)
{
    bool        *closed = (bool*)closure;
    const char  *err    = NULL;
...
static void
nats_connectioned(natsConnection *nc, void *closure)
{
    bool        *closed = (bool*)closure;

    flb_warn("[STAN] NATS connectioned");
    *closed = false;
}
...
static int configure_nats(struct flb_out_stan_config **config) {
    ...
    if (natsOptions_Create(&ctx->nats_options) != NATS_OK) {
        flb_error("[STAN] NATS Error (%d) when creating options: '%s'", ctx->status, natsStatus_GetText(ctx->status));
        return 1;
    }
    ...
    ctx->status = natsOptions_SetRetryOnFailedConnect(ctx->nats_options, true, nats_connectioned, (void*)&ctx->nats_closed);
    if (ctx->status != NATS_OK) {
        flb_error(nats_setting_error, "Retry on failed connect callback", ctx->status, natsStatus_GetText(ctx->status));
        return(1);
    }

    ctx->status = natsOptions_SetClosedCB(ctx->nats_options, nats_connection_lost, (void*)&ctx->nats_closed);
    if (ctx->status != NATS_OK) {
...
static int configure_stan(struct flb_out_stan_config **config) {
    ...
    if (ctx->status != NATS_OK) {
        flb_error(nats_setting_error, "Connection lost handler", ctx->status, natsStatus_GetText(ctx->status));
        return(1);
    }
    ...
    if (ctx->status != NATS_OK) {
        flb_error(nats_setting_error, "Pings", ctx->status, natsStatus_GetText(ctx->status));
        return(1);
    }

    ctx->status = stanConnOptions_SetNATSOptions(ctx->stan_options, ctx->nats_options);
    if (ctx->status != NATS_OK) {
        flb_error(nats_setting_error, "STAN options with NATS options", ctx->status, natsStatus_GetText(ctx->status));
        return(1);
    }
    return 0;
}
...
int cb_stan_init(struct flb_output_instance *ins, struct flb_config *config, void *data)
{
    ...
    //if (configure_nats(&ctx)) { return 1; }
    //if (configure_stan(&ctx)) { return 1; }
...
void cb_stan_flush(void *data, size_t bytes,
...
struct flb_output_plugin out_stan_plugin = {
    ...
    .cb_init      = NULL, //cb_stan_init,
    .cb_flush     = NULL, //cb_stan_flush,
    .cb_exit      = NULL, //cb_stan_exit,
    .flags        = 0,
};
