        ...
        for (i = 0; i < map_size; i++) {
            ...
            if (check == FLB_TRUE) {
                if (strncmp(ctx->log_key, key_str, key_str_size) == 0) {
                    ...
                    ret = flb_msgpack_to_json(val_buf, bytes + bytes / 4, &val);
                    ...
                    /* Remove double quotes and add newline at end */
                    ...
                    /* Free memory allocated to store record contents */
                    flb_free(orig_val_buf);
                }
            }
        }
                            ...
                            struct flb_config *config)
{
    ...
    if (ctx->log_key) {
        json = flb_pack_msgpack_extract_log_key(ctx, data, bytes);
    } else {
