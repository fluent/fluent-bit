...
static inline int throttle_data(struct flb_filter_throttle_ctx *ctx, long timestamp)
{
    if (timestamp == ctx->hash->current_timestamp && ctx->hash->total / ctx->hash->size > ctx->max_rate) {
        return THROTTLE_RET_DROP;
    }

    window_add(ctx->hash, timestamp, 1);
                          ...
                          struct flb_config *config)
{
    ...
    while (msgpack_unpack_next(&result, data, bytes, &off)) {
        ...
        ret = throttle_data(context, timestamp);
