...
#include "in_ebpf.h"
#include "traces/registry.h"

int trace_register(struct flb_in_ebpf_context *ctx, const char *name, struct bpf_object *obj, trace_event_handler_t handler) {
    struct trace_context *trace;
    int map_fd;

    flb_plg_trace(ctx->ins, "Registering trace handler for: %s", name);
    ...
    bpf_object__for_each_map(map, obj) {
        flb_plg_info(ctx->ins, "Found BPF map: %s", bpf_map__name(map));
    }

    struct bpf_map *events_map = bpf_object__find_map_by_name(obj, ctx->ringbuf_map_name);
    if (!events_map) {
        flb_plg_error(ctx->ins, "Could not find '%s' map in BPF object for trace: %s", ctx->ringbuf_map_name, name);
        return -1;
    }

    map_fd = bpf_map__fd(events_map);
    if (map_fd < 0) {
        flb_plg_error(ctx->ins, "Failed to get file descriptor for '%s' map for trace: %s", ctx->ringbuf_map_name, name);
        return -1;
    }
...
int trace_setup(struct flb_in_ebpf_context *ctx, const char *trace_name) {
    ...
    flb_plg_trace(ctx->ins, "Setting up trace configuration for: %s", trace_name);
    ...
    for (reg = trace_table; reg->name != NULL; reg++) {
        ...
        flb_plg_trace(ctx->ins, "Attaching BPF program for trace: %s", trace_name);
        ...
        int ret = trace_register(ctx, trace_name, obj, reg->handler);
        if (ret != 0) {
...
static int in_ebpf_collect(struct flb_input_instance *ins, struct flb_config *config, void *in_context) {
    ...
    flb_plg_trace(ins, "Collecting events from ring buffers");

    for (int i = 0; i < ctx->trace_count; i++) {
        flb_plg_trace(ctx->ins, "Consuming events from ring buffer %s", ctx->traces[i].name);
        int err = ring_buffer__consume(ctx->traces[i].rb);
        if (err < 0) {
            flb_plg_error(ins, "Error consuming from ring buffer: %d", err);
        } else {
            flb_plg_trace(ins, "Successfully consumed events from ring buffer %s", ctx->traces[i].name);
        }
    }
...
static int in_ebpf_init(struct flb_input_instance *ins, struct flb_config *config, void *data) {
    ...
    flb_plg_trace(ins, "Initializing eBPF input plugin");
    ...
    mk_list_foreach(head, &ins->properties) {
        ...
        if (strcasecmp(kv->key, "trace") == 0) {
            ...
            flb_plg_trace(ctx->ins, "Processing trace: %s", trace_name);
            err = trace_setup(ctx, trace_name);
            if (err != 0) {
    ...
    flb_plg_trace(ctx->ins, "Setting up collector with poll interval: %d ms", ctx->poll_ms);

    ctx->coll_fd = flb_input_set_collector_time(ins, in_ebpf_collect, poll_seconds, poll_nanoseconds, config);
    if (ctx->coll_fd < 0) {
...
static void in_ebpf_pause(void *data, struct flb_config *config) {
    ...
    flb_plg_trace(ctx->ins, "Collector paused");
}

static void in_ebpf_resume(void *data, struct flb_config *config) {
    struct flb_in_ebpf_context *ctx = data;
    flb_input_collector_resume(ctx->coll_fd, ctx->ins);
    flb_plg_trace(ctx->ins, "Collector resumed");
}
...
static struct flb_config_map config_map[] = {
    {FLB_CONFIG_MAP_STR, "ringbuf_map_name", FLB_IN_EBPF_DEFAULT_RINGBUF_MAP_NAME, 0, FLB_TRUE, offsetof(struct flb_in_ebpf_context, ringbuf_map_name)},
    {FLB_CONFIG_MAP_INT, "poll_ms", FLB_IN_EBPF_DEFAULT_POLL_MS, 0, FLB_TRUE, offsetof(struct flb_in_ebpf_context, poll_ms)},
