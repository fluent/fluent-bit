...
cfl_sds_t cm_key_at(struct content_modifier_ctx *ctx, size_t index)
{
    ...
    if (ctx->keys != NULL && mk_list_is_empty(ctx->keys) != 0) {
        ...
        mk_list_foreach(head, ctx->keys) {
            if (current_index == index) {
                entry = mk_list_entry(head, struct flb_config_map_val, _head);
...
static int set_context(struct content_modifier_ctx *ctx)
{
    ...
    if (event_type == FLB_PROCESSOR_LOGS) {
         ...
         */
            ...
            else if (cm_key_count(ctx) != 1 || strcasecmp(cm_key_at(ctx, 0), "name") != 0) {
                flb_plg_error(ctx->ins, "context '%s' requires scalar key 'name'",
                              ctx->context_str);
                return -1;
            }
            ...
            else if (cm_key_count(ctx) != 1 || strcasecmp(cm_key_at(ctx, 0), "version") != 0) {
                flb_plg_error(ctx->ins, "context '%s' requires scalar key 'version'",
                              ctx->context_str);
                return -1;
            }
...
static int check_action_requirements(struct content_modifier_ctx *ctx)
{
    ...
    if (cm_key_count(ctx) > 1 &&
        (ctx->action_type == CM_ACTION_RENAME || ctx->action_type == CM_ACTION_EXTRACT)) {
        flb_plg_error(ctx->ins, "action '%s' requires a scalar key", ctx->action_str);
        return -1;
    }
