...
static int splunk_conn_realloc_and_reparse(struct flb_splunk *ctx,
                                          struct splunk_conn *conn,
                                          size_t size)
{
    char *tmp;

    flb_plg_debug(ctx->ins, "realloc buffer %i -> %zu bytes",
    ...
    flb_plg_debug(ctx->ins, "realloc completed successfully size=%zu", size);
    return 0;
}
...
static int splunk_conn_event(void *data)
{
    ...
    if (event->mask & MK_EVENT_READ) {
        ...
        if (available < 1) {
            ...
            ret = splunk_conn_realloc_and_reparse(ctx, conn, size);
            if (ret == -1) {
                flb_errno();
                return -1;
            }
