...
struct flb_test_out_response {
     ...
     * - rt_ffd : this plugin assigned 'integer' created by flb_output()
     *
     * - rt_response_calback: intermediary function to receive the response of
     *                        the output plugin test function.
     *
     * - rt_data: opaque data type for rt_step_callback()
     */
     ...
     * code, it's set on src/flb_lib.c .
     */
    void (*rt_response_callback) (void *, int, int, void *, size_t, void *);
     ...
     * Callback
     * =========
     * "response callback": it references the plugin function that performs
     * receiving response which is mostly HTTP requests.
     * This entry is mostly to expose the plugin local function.
     */
    int (*callback) (/* Fluent Bit context */
                     struct flb_config *,
                     void *,         /* plugin instance context */
                     const char *,   /* incoming response data */
