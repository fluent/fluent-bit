...
static int ctr_protobuf_read_varint(struct ctr_protobuf_frame *frame, uint64_t *value)
{
    unsigned int shift;
    unsigned char byte;
 ...
 */
static int ctr_protobuf_validate(const ProtobufCMessageDescriptor *descriptor,
                                    const void *data, size_t size)
{
    struct ctr_protobuf_frame frames[CTR_PROTOBUF_MAX_DEPTH];
    struct ctr_protobuf_frame *frame;
    ...
    while (depth > 0) {
        ...
        if (ctr_protobuf_read_varint(frame, &tag) != 0 || tag >> 3 == 0 || tag >> 3 > 0x1fffffff) {
            return -1;
        }

        switch (tag & 7) {
        case 0:
            if (ctr_protobuf_read_varint(frame, &length) != 0) {
                return -1;
            }
            continue;
        case 1:
        ...
        if (depth >= CTR_PROTOBUF_MAX_DEPTH) {
            return -1;
        }
