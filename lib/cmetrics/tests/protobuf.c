...
static void test_protobuf_depth(void)
{
    ...
    TEST_CHECK(cmt_protobuf_validate(values[1].base.descriptor, buffer, size) == 0);
    size = opentelemetry__proto__common__v1__array_value__pack(&arrays[0], buffer);
    TEST_CHECK(cmt_protobuf_validate(arrays[0].base.descriptor, buffer, size) == 0);
    size = opentelemetry__proto__common__v1__any_value__pack(&values[0], buffer);
    TEST_CHECK(cmt_protobuf_validate(values[0].base.descriptor, buffer, size) != 0);
}
...
static void test_protobuf_wire_boundaries(void)
{
    ...
    TEST_CHECK(cmt_protobuf_validate(descriptor, NULL, 0) == 0);
    TEST_CHECK(cmt_protobuf_validate(descriptor, truncated, sizeof(truncated)) != 0);
    TEST_CHECK(cmt_protobuf_validate(descriptor, overflow, sizeof(overflow)) != 0);
    TEST_CHECK(cmt_protobuf_validate(descriptor, opaque, sizeof(opaque)) == 0);
    TEST_CHECK(cmt_protobuf_validate(descriptor, unknown, sizeof(unknown)) == 0);
    TEST_CHECK(cmt_protobuf_validate(descriptor, zero_tag, sizeof(zero_tag)) != 0);
    TEST_CHECK(cmt_protobuf_validate(descriptor, fixed, sizeof(fixed)) == 0);
    TEST_CHECK(cmt_protobuf_validate(descriptor, fixed, sizeof(fixed) - 1) != 0);
}
