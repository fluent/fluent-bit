...
#ifndef CPROF_PROTOBUF_H
#define CPROF_PROTOBUF_H

#include <fluent-otel-proto/fluent-otel.h>

/* Budget includes export/resource/scope wrappers and scalar AnyValue messages. */
#define CPROF_PROTOBUF_MAX_DEPTH 100

struct cprof_protobuf_frame {
    const ProtobufCMessageDescriptor *descriptor;
    const unsigned char *cursor;
    const unsigned char *end;
};

static int cprof_protobuf_read_varint(struct cprof_protobuf_frame *frame, uint64_t *value)
{
    unsigned int shift;
    unsigned char byte;
 ...
 */
static int cprof_protobuf_validate(const ProtobufCMessageDescriptor *descriptor,
                                    const void *data, size_t size)
{
    struct cprof_protobuf_frame frames[CPROF_PROTOBUF_MAX_DEPTH];
    struct cprof_protobuf_frame *frame;
    ...
    while (depth > 0) {
        ...
        if (cprof_protobuf_read_varint(frame, &tag) != 0 || tag >> 3 == 0 || tag >> 3 > 0x1fffffff) {
            return -1;
        }

        switch (tag & 7) {
        case 0:
            if (cprof_protobuf_read_varint(frame, &length) != 0) {
                return -1;
            }
            continue;
        case 1:
        ...
        if (depth >= CPROF_PROTOBUF_MAX_DEPTH) {
            return -1;
        }
