...
static int handle_return(mbedtls_ssl_context *ssl, int ret)
{
    struct polar_context_head *head;

    head = context_get_head(*(int *) ssl->p_bio);
    if (head != NULL) {
    ...
    if (ret < 0) {
        ...
        MK_TRACE("[tls] SSL error: %s", err_buf);
    }
#endif
    if (ret < 0) {
        switch( ret )
                        ...
                        struct polar_config *conf)
{
    ...
    check_client_cert = (size_t) mk_rconf_section_get_key(section,
                                                          "CheckClientCert",
                                                          MK_RCONF_BOOL);
fallback:
    ...
    if (cert_chain_file) {
        ...
        if (*cert_chain_file == '/') {
            ...
            mk_string_build(&conf->cert_chain_file, &len,
    ...
    /* Set default name if not specified */
    if (!key_file) {
        mk_string_build(&conf->key_file, &len,
                        "%s/rsa.pem", confdir);
    }
    else {
        /* Set absolute path or compose a new one based on the relative */
    ...
    /* Set default name if not specified */
    if (!dh_param_file) {
        mk_string_build(&conf->dh_param_file, &len,
                        "%s/dhparam.pem", confdir);
    }
    else {
        /* Set absolute path or compose a new one based on the relative */
        if (*dh_param_file == '/') {
...
static int tls_global_init()
{
    ...
    MK_TRACE("[tls] Load certificates.");
    if (polar_load_certs(&server_context->config)) {
        return -1;
    }
    MK_TRACE("[tls] Load DH parameters.");
...
static mbedtls_ssl_context *context_new(int fd)
{
    ...
    if (*cur == NULL) {
        MK_TRACE("[polarssl %d] New ssl context.", fd);

        *cur = mk_mem_alloc(sizeof(**cur));
        if (*cur == NULL) {
            return NULL;
        }
        (*cur)->_next = NULL;
        (*cur)->want_event = MK_EVENT_READ;
...
static int tls_event_interest(struct mk_plugin *plugin, int fd, int fallback)
{
    ...
    if (head->want_event == MK_EVENT_EMPTY) {
        return fallback;
    }

    return head->want_event;
}
...
static int mk_tls_close(struct mk_plugin *plugin, int fd)
{
    ...
    MK_TRACE("[fd %d] Closing connection", fd);
...
void mk_tls_thread_init(struct mk_server *server)
{
    ...
    MK_TRACE("[tls] Init thread context.");

    thctx = mk_mem_alloc(sizeof(*thctx));
    if (thctx == NULL) {
        goto error;
    }
    thctx->contexts = NULL;
    ...
    MK_TRACE("[tls] Load RSA key.");
    if (polar_load_key(thctx, &server_context->config)) {
        goto error;
    }

    MK_TRACE("[tls] Set local thread context.");
...
void mk_tls_exit(struct mk_server *server)
{
    ...
    mk_list_foreach_safe(cur, tmp, &server_context->threads._head) {
        ...
        mbedtls_ctr_drbg_free(&thctx->ctr_drbg);
        mbedtls_ssl_config_free(&thctx->conf);
        mk_mem_free(thctx);
    }
    mbedtls_entropy_free(&server_context->entropy);
