...
int flb_router_apply_config(struct flb_config *config)
{
    ...
    /* Simple approach: find dummy input and stdout output and connect them */
    input_ins = NULL;
    mk_list_foreach(i_head, &config->inputs) {
        input_ins = mk_list_entry(i_head, struct flb_input_instance, _head);
        if (strncmp(input_ins->name, "dummy", 5) == 0) {
            break;
        }
        input_ins = NULL;
    }
    ...
    mk_list_foreach(o_head, &config->outputs) {
        output_ins = mk_list_entry(o_head, struct flb_output_instance, _head);
        /* Check by name or alias */
        if (strncmp(output_ins->name, "stdout", 6) == 0 ||
            (output_ins->alias && strcmp(output_ins->alias, "loki") == 0)) {
            break;
        }
        output_ins = NULL;
    }
    ...
    if (input_ins && output_ins) {
        flb_debug("[router] connecting dummy input to stdout output (alias: loki)");
        flb_router_connect_direct(input_ins, output_ins);
        flb_debug("[router] connection established");
    } else {
        flb_warn("[router] could not find dummy input or stdout output");
    }
