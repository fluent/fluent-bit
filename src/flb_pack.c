...
flb_sds_t flb_pack_msgpack_to_json_format_with_trace(const char *data, uint64_t bytes,
                                          int json_format, int date_format,
                                          flb_sds_t date_key)
{
    int i;
    ...
    while (msgpack_unpack_next(&result, data, bytes, &off) == ok) {
        ...
        if (root.via.array.size > 2) {
            msgpack_pack_str_with_body(&tmp_pck, "trace", strlen("trace"));
            msgpack_pack_array(&tmp_pck, root.via.array.size-2);
            for (i = 2; i < root.via.array.size; i++) {
                msgpack_pack_object(&tmp_pck, root.via.array.ptr[i]);
            }
        }
