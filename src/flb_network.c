...
void flb_net_setup_init(struct flb_net_setup *net)
{
    ...
    net->dns_preferred_address_family = NULL;
                                 ...
                                 struct flb_upstream_conn *u_conn)
{
    ...
    if (u_conn->u->net.dns_preferred_address_family != NULL) {
        preferred_address_family = -1;

        if (strlen(u_conn->u->net.dns_preferred_address_family) > 3) {
            if (u_conn->u->net.dns_preferred_address_family[3] == '4') {
                preferred_address_family = AF_INET;
            }
            else if (u_conn->u->net.dns_preferred_address_family[3] == '6') {
                preferred_address_family = AF_INET6;
            }
        }

        if (preferred_address_family != -1) {
            sorted_res = flb_net_sort_addrinfo_list(res, preferred_address_family);
