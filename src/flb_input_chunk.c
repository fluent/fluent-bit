                                          ...
                                          size_t chunk_size)
{
    ...
    mk_list_foreach(head, &ic->in->chunks) {
        ...
        count++;
        flb_info("[input chunk] chk: %s - Updated bytes remained: %d. Count: %d. Chunk: %s. Old_ic: %s",
                 flb_input_chunk_get_name(ic), bytes_remained, count, flb_input_chunk_get_name(ic), flb_input_chunk_get_name(old_ic));
                                        ...
                                        size_t chunk_size, int overlimit)
{
    ...
    mk_list_foreach(head, &ic->in->config->outputs) {
        ...
        flb_info("[input chunk] chk: %s - Output instance: %s. Total_limit_size: %d. Over-limit %d",
                 flb_input_chunk_get_name(ic), o_ins->name, o_ins->total_limit_size, overlimit);
        ...
        if (!result && local_release_requirement == 0) {
             ...
             */
            flb_info("[input chunk] chk: %s - space requirement was satisfied for chunk in output %s", flb_input_chunk_get_name(ic), o_ins->name);
            continue;
        }
        ...
        if (count == 0) {
             ...
             */
            flb_error("[input chunk] chk: %s - no enough space in filesystem to buffer "
                      "chunk %s in plugin %s", flb_input_chunk_get_name(ic), flb_input_chunk_get_name(ic), o_ins->name);
