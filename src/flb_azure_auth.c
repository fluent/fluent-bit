                                          ...
                                          const char *resource)
{
    ...
    /* Clean up the body sds now that the request is done */
    flb_sds_destroy(body);
    body = NULL;
    ...
    if (c->resp.payload_size > 0) {
        if (c->resp.status == 200) {
            ...
            flb_warn("[azure workload identity] token exchange failed with status %i", c->resp.status);
            flb_http_client_destroy(c);
            flb_upstream_conn_release(u_conn);
            flb_sds_destroy(federated_token);
            return -1;
    ...
    if (c->resp.payload_size > 0 && c->resp.status == 200) {
        /* Acquire lock before updating token */
        ret = flb_lock_acquire(&ctx->lock,
                               FLB_LOCK_DEFAULT_RETRY_LIMIT,
                               FLB_LOCK_DEFAULT_RETRY_DELAY);
        if (ret != 0) {
            flb_error("[azure workload identity] failed to acquire lock");
            flb_http_client_destroy(c);
            flb_upstream_conn_release(u_conn);
            flb_sds_destroy(federated_token);
            return -1;
