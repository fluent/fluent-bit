...
char *flb_azure_msi_token_get(struct flb_oauth2 *ctx)
{
    ...
    /* Remove default buffer size limit - token responses can exceed 4KB */
...
static flb_sds_t read_token_from_file(const char *token_file)
{
    ...
    if (ferror(fp)) {
        fclose(fp);
        flb_error("[azure workload identity] error reading token file: %s", token_file);
        return NULL;
    }
    fclose(fp);
                                          ...
                                          const char *resource)
{
    ...
    if (flb_sds_cat_safe(&body, "client_id=", 10) < 0 ||
        flb_sds_cat_safe(&body, client_id, strlen(client_id)) < 0 ||
        flb_sds_cat_safe(&body, "&grant_type=client_credentials",
                         sizeof("&grant_type=client_credentials") - 1) < 0 ||
        flb_sds_cat_safe(&body, "&client_assertion_type="
                         "urn:ietf:params:oauth:client-assertion-type:jwt-bearer",
                         sizeof("&client_assertion_type="
                                "urn:ietf:params:oauth:client-assertion-type:jwt-bearer") - 1) < 0 ||
        flb_sds_cat_safe(&body, "&client_assertion=",
                         sizeof("&client_assertion=") - 1) < 0 ||
        flb_sds_cat_safe(&body, federated_token, flb_sds_len(federated_token)) < 0 ||
        flb_sds_cat_safe(&body, "&scope=", sizeof("&scope=") - 1) < 0 ||
        flb_sds_cat_safe(&body, resource, strlen(resource)) < 0) {
