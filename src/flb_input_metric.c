                                ...
                                struct cmt *cmt)
{
    ...
    if (mt_size <= FLB_INPUT_CHUNK_FS_MAX_SIZE) {
        ...
        if (ret == 0 && tag != NULL) {
            ...
            /*
             * Keep metric chunks short-lived per append. Reusing the same
             * tag-bound metric chunk can delay task creation for rapidly
             * updated series, which makes runtime consumers miss freshly
             * generated metrics.
             */
            pthread_mutex_lock(&ins->metrics_chunk_lock);
            chunk_ref = flb_hash_table_get_ptr(ins->ht_metric_chunks,
                                               tag, tag_len);
            if (chunk_ref != NULL) {
