...
int flb_digest_init(struct flb_digest *context, int hash_type)
{
...
int flb_digest_finalize(struct flb_digest *context,
...
int flb_digest_update(struct flb_digest *context,
                      unsigned char *data,
                      size_t data_length)
{
    int result;
...
int flb_digest_cleanup(struct flb_digest *context)
{
    if (context->backend_context == NULL) {
        return FLB_CRYPTO_INVALID_ARGUMENT;
    }
...
int  flb_digest_simple_batch(int hash_type,
                             size_t entry_count,
                             ...
                             size_t digest_buffer_size)
{
    struct flb_digest digest_context;
    size_t            entry_index;
    int               result;

    result = flb_digest_init(&digest_context, hash_type);
    ...
    if (result == FLB_CRYPTO_SUCCESS) {
        ...
        if (result == FLB_CRYPTO_SUCCESS) {
            result = flb_digest_finalize(&digest_context,
                                         digest_buffer,
                                         digest_buffer_size);
        }

        flb_digest_cleanup(&digest_context);
    }

    return result;
}

int flb_digest_simple(int hash_type,
                      unsigned char *data,
                      size_t data_length,
                      unsigned char *digest_buffer,
                      size_t digest_buffer_size)
{
    ...
    return flb_digest_simple_batch(hash_type,
                                   1,
                                   data_entries,
                                   length_entries,
                                   digest_buffer,
