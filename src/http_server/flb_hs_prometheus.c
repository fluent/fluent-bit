...
flb_sds_t metrics_help_txt(char *metric_name, flb_sds_t *metric_helptxt)
{
    ...
    else if (strstr(metric_name, "storage_layer_total_chunks")) {
        return flb_sds_cat(*metric_helptxt, " Storage layer total chunks.\n", 29);
    }
    else if (strstr(metric_name, "storage_layer_mem_chunks")) {
        return flb_sds_cat(*metric_helptxt, " Storage layer memory chunks.\n", 30);
    }
    else if (strstr(metric_name, "storage_layer_fs_chunks")) {
        return flb_sds_cat(*metric_helptxt, " Storage layer fs chunks.\n", 26);
    }
    else if (strstr(metric_name, "storage_layer_fs_chunks_up")) {
        return flb_sds_cat(*metric_helptxt, " Storage layer fs chunks up.\n", 29);
    }
    else if (strstr(metric_name, "storage_layer_fs_chunks_down")) {
        return flb_sds_cat(*metric_helptxt, " Storage layer fs chunks down.\n", 31);
    }
    else if (strstr(metric_name, "input_chunks_status_overlimit")) {
        return flb_sds_cat(*metric_helptxt, " Overlimit is reached.\n", 23);
    }
    else if (strstr(metric_name, "input_chunks_status_mem_size_bytes")) {
        return flb_sds_cat(*metric_helptxt, " Actual memory size in bytes.\n", 30);
    }
    else if (strstr(metric_name, "input_chunks_status_mem_size")) {
        return flb_sds_cat(*metric_helptxt, " Actual memory size adjusted to human readable.\n", 48);
    }
    else if (strstr(metric_name, "input_chunks_status_mem_limit_bytes")) {
        return flb_sds_cat(*metric_helptxt, " Memory limit in bytes.\n", 24);
    }
    else if (strstr(metric_name, "input_chunks_status_mem_limit")) {
        return flb_sds_cat(*metric_helptxt, " Memory limit adjusted to human readable.\n", 42);
    }
    else if (strstr(metric_name, "input_chunks_chunks_total")) {
        return flb_sds_cat(*metric_helptxt, " Number of total chunks.\n", 25);
    }
    else if (strstr(metric_name, "input_chunks_chunks_up")) {
        return flb_sds_cat(*metric_helptxt, " Number of up chunks.\n", 22);
    }
    else if (strstr(metric_name, "input_chunks_chunks_down")) {
        return flb_sds_cat(*metric_helptxt, " Number of down chunks.\n", 24);
    }
    else if (strstr(metric_name, "input_chunks_chunks_busy_size_bytes")) {
        return flb_sds_cat(*metric_helptxt, " Number of busy size chunks in bytes.\n", 38);
    }
    else if (strstr(metric_name, "input_chunks_chunks_busy_size")) {
        return flb_sds_cat(*metric_helptxt, " Number of busy size chunks adjusted to human readable.\n", 56);
    }
    else if (strstr(metric_name, "input_chunks_chunks_busy")) {
        return flb_sds_cat(*metric_helptxt, " Number of busy chunks.\n", 24);
    }
    else if (strstr(metric_name, "global_overlimit")) {
        return flb_sds_cat(*metric_helptxt, " Global overlimit is reached.\n", 30);
    }
    else {
