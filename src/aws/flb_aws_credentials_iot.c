                                                 ...
                                                 *generator)
{
     ...
     * default to https:// if missing.  Reject other schemes early.
     */
    if (strncmp(credentials_endpoint, "http://", 7) == 0 ||
        strncmp(credentials_endpoint, "https://", 8) == 0) {
        /* Supported scheme already present; nothing to do */
    }
    else if (strstr(credentials_endpoint, "://") != NULL) {
        AWS_CREDS_ERROR("Unsupported scheme in IoT credentials endpoint "
    ...
    if (port_sds && flb_sds_len(port_sds) > 0) {
        char *endptr = NULL;
        long port_long;
