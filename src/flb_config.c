...
int flb_config_load_config_format(struct flb_config *config, struct flb_cf *cf)
{
    ...
    printf("[DEBUG] finished plugin configuration, starting router parsing\n");
    flb_debug("[config] finished plugin configuration, starting router parsing");
    /* Parse new router configuration */
    printf("[DEBUG] starting router configuration parsing\n");
    flb_debug("[router] starting router configuration parsing");
    ret = flb_router_config_parse(cf, &config->input_routes, config);
    if (ret == -1) {
        printf("[DEBUG] router configuration parsing failed\n");
        flb_debug("[router] router configuration parsing failed");
        return -1;
    }

    /* Debug: Print parsed router configuration */
    if (!cfl_list_is_empty(&config->input_routes)) {
        printf("[DEBUG] parsed %d input routes\n", cfl_list_size(&config->input_routes));
        flb_debug("[router] parsed %d input routes", cfl_list_size(&config->input_routes));
    } else {
        printf("[DEBUG] no input routes parsed\n");
        flb_debug("[router] no input routes parsed");
    }
