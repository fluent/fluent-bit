 ...
 */
static void headers_sanitize_ng(struct flb_http_request *request, struct mk_list *out_list)
{
    int x;
    char *v_start;
    char *v_end;
    ...
    mk_list_foreach(head, &request->headers->entries) {
        header_entry = mk_list_entry(head,
 ...
 */
static flb_sds_t flb_signv4_ng_canonical_request(struct flb_http_request *request,
                                                 int normalize_uri,
                                                 int amz_date_header,
                                                 char *amzdate,
                                                 char *security_token,
                                                 ...
                                                 flb_sds_t *signed_headers)
{
    ...
    size = strlen(request->path) +
           (request->headers->total_count * 64) +
           256;
    ...
    if (normalize_uri == FLB_TRUE) {
        tmp = flb_signv4_uri_normalize_path((char *) request->path,
                                            strlen(request->path));
        if (!tmp) {
    ...
    if (request->query_string) {
        params = url_params_format((char *) request->query_string);
        if (!params) {
            flb_sds_destroy(cr);
            return NULL;
        }
    ...
    if (request->method == HTTP_METHOD_POST &&
        (request->body != NULL && cfl_sds_len(request->body) > 0)) {
        val = (char *) flb_http_request_get_header(request,  "Content-Type");

        if (val) {
            if (strstr(val, "application/x-www-form-urlencoded")) {
                params = url_params_format((char *) request->body);
                if (!params) {
    ...
    if (s3_mode == S3_MODE_UNSIGNED_PAYLOAD) {
        ...
        if ((request->body != NULL && cfl_sds_len(request->body) > 0) &&
            post_params == FLB_FALSE) {
            result = flb_hash_simple(FLB_HASH_SHA256,
                                     (unsigned char *) request->body,
    ...
    if (s3_mode == S3_MODE_SIGNED_PAYLOAD) {
        flb_http_request_set_header(request,
                                    "-amz-content-sha256", 0,
                                    payload_hash, 0);
    }

    headers_sanitize_ng(request, &list_tmp);
 ...
 */
static flb_sds_t flb_signv4_string_to_sign(struct flb_http_request *request,
 ...
 */
static flb_sds_t flb_signv4_add_authorization(struct flb_http_request *request,
                                              char *access_key,
                                              char *datestamp,
                                              ...
                                              flb_sds_t signature)
{
    ...
    ret = flb_http_request_set_header(request,
                                      "Authorization", 0,
                                      header_value, 0);
                           ...
                           struct flb_aws_provider *provider)
{
    ...
    cr = flb_signv4_ng_canonical_request(request, normalize_uri,
    ...
    string_to_sign = flb_signv4_string_to_sign(request, cr, amzdate,
    ...
    auth_header = flb_signv4_add_authorization(request,
                                               creds->access_key_id,
                                               datestamp, region, service,
                                               signed_headers, signature);
    flb_sds_destroy(signed_headers);
