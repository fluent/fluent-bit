...
static struct cfl_kvpair *cfl_object_kvpair_get(struct cfl_object *obj, cfl_sds_t key)
{
    ...
    switch (obj->type) {
    case CFL_OBJECT_NONE:
    case CFL_OBJECT_VARIANT:
    case CFL_OBJECT_ARRAY:
        return NULL;
    case CFL_OBJECT_KVLIST:
        break;
    }

    kvlist = obj->variant->data.as_kvlist;
    /* We need to traverse the elements of list */
...
struct flb_cobj_ra_value *flb_cobj_ra_key_to_value(flb_sds_t ckey,
                                                   ...
                                                   struct mk_list *subkeys)
{
    ...
    struct flb_cobj_ra_value *result = NULL;

    /* Get the kvpair in the variant */
    kvpair = cfl_object_kvpair_get(&cobj, ckey);
    if (kvpair == NULL) {
        return NULL;
    }
    ...
    result = flb_calloc(1, sizeof(struct flb_cobj_ra_value));
    if (!result) {
        flb_errno();
        return NULL;
    }
...
int flb_cobj_ra_key_value_get(flb_sds_t ckey, struct cfl_object cobj,
                              struct mk_list *subkeys,
                              cfl_sds_t *start_key,
                              cfl_sds_t *out_key, struct cfl_variant **out_val)
{
...
void flb_cobj_ra_key_value_destroy(struct flb_cobj_ra_value *v)
{
    flb_free(v);
}
...
int flb_cobj_ra_key_strcmp(flb_sds_t ckey, struct cfl_object cobj,
...
int flb_cobj_ra_key_regex_match(flb_sds_t ckey, struct cfl_object cobj,
...
int flb_cobj_ra_key_value_update(struct flb_ra_parser *rp,  struct cfl_object *cobj,
                                 cfl_sds_t in_key, struct cfl_variant *in_val)
{
    int i;
    int kv_size;
    ...
    /* Get the kvpair in the cfl_object */
    kvpair = cfl_object_kvpair_get(cobj, rp->key->name);
    if (kvpair == NULL) {
        return -1;
    }

    if (cobj->type != CFL_OBJECT_KVLIST) {
        return -1;
    }
    kvlist = cobj->variant->data.as_kvlist;
...
int flb_cobj_ra_key_value_append(struct flb_ra_parser *rp, struct cfl_object *cobj,
                                 struct cfl_variant *in_val)
{
    struct cfl_kvlist *kvlist;
    struct cfl_kvpair *kvpair;
